# Welcome CuongDNSE130489 from Using Databases with Python
#
...
# there is a balance between the number of operations you execute between commits and the importance of
...
for line in data :
    if line.startswith('From') and not line.startswith('From:') :
        sliceLine = line.strip().split()
        domainEmail = sliceLine[1]
        emailTemp = domainEmail.split("@")
        email = emailTemp[1]
        cur.execute('SELECT count From Counts Where org = ?', (email, ))
        row = cur.fetchone()
        if row is None :
            cur.execute('''
            INSERT INTO Counts(org, count) VALUES(?, 1)''', (email, ))
        else :
            # (countTemp1, ) =  cur.execute('Select count From Counts ORDER BY count DESC LIMIT 1 ')
            # countTemp = countTemp1[0]
            # print(countTemp)
            cur.execute('''
            Update Counts Set count = count + 1 Where org = ?''', (email, ))
conn.commit()
